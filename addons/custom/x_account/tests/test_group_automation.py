...
class TestXGroupAutomation(XAccountTestBase):
    ...
    def setUpClass(cls):
        ...
        # Deactivate the real base_automation rules so tests exercise
        # `_enqueue_group_operation` + the queue worker in isolation (no
        # double-enqueue on create, no auto-execution before the test patches
        # the provider).
        for xmlid in (
