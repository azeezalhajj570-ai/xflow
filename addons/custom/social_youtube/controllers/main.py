...
class SocialYoutubeController(SocialController):
    ...
    def youtube_account_callback(self, code=None, iap_access_token=None, iap_refresh_token=None, iap_expires_in=0, **kw):
        ...
        if iap_access_token and iap_refresh_token:
            ...
            _logger.info(
                'youtube_callback: exchange code base_url=%s redirect_uri=%s client_id=%s',
                request.httprequest.host_url, redirect_uri, youtube_oauth_client_id and youtube_oauth_client_id[:30] + '...',
            )
            ...
            _logger.info('youtube_callback: token response: %s', json.dumps(token_exchange_response))
            ...
            if not token_exchange_response.get('refresh_token'):
                _logger.warning(
                    'youtube_callback: no refresh_token in response, keys=%s',
                    list(token_exchange_response.keys()),
                )
                return request.render('social.social_http_error_view', {
