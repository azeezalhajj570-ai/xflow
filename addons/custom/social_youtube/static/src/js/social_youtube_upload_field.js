...
export class YoutubeUploadField extends CharField {
    ...
    async _openUploadSession(fileSize, fileType) {
        ...
        const response = await fetch('https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=status%2Csnippet', {
            method: 'POST',
            headers: {
                "Authorization": "Bearer " + data.youtube_access_token,
        ...
        if (!response.ok) {
            ...
            try {
                const errorReason = JSON.parse(errorText).error?.errors?.[0]?.reason;
    ...
    async _updateProcessingInfo() {
        try {
            const response = await fetch('https://www.googleapis.com/youtube/v3/videos?part=processingDetails&id=' + this.uploadedVideoId, {
                method: 'GET',
                headers: {
            ...
            if ('items' in json && json.items.length === 1 && 'processingDetails' in json.items[0]) {
                const processingDetails = json.items[0].processingDetails;
    ...
    _onClearClick() {
        this.dialogService.add(ConfirmationDialog, {
            ...
            confirm: () => {
                const accessToken = this.props.record.data.youtube_access_token;
                const videoId = this.props.record.data.youtube_video_id;
